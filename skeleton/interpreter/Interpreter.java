...
import ast.Expr;
import ast.ExprList;
import ast.FormalDeclList;
import ast.FuncDef;
import ast.IdentExpr;
import ast.IfElseStmt;
import ast.IfStmt;
import ast.LogicalCond;
import ast.NeExprList;
import ast.NeFormalDeclList;
import ast.NilExpr;
...
import ast.UnaryCond;
import ast.UnaryExpr;
import ast.UpdateStmt;
import ast.VarDecl;
        ...
        if (cond instanceof BinaryCond) {
            ...
            switch (logicalCond.getOperator()) {
                case LogicalCond.AND:
                    return evaluateCond(logicalCond.getLeft(), func) && evaluateCond(logicalCond.getRight(), func);
                case LogicalCond.OR:
                    return evaluateCond(logicalCond.getLeft(), func) || evaluateCond(logicalCond.getRight(), func);
                default:
                    throw new RuntimeException("Unhandled operator");
            ...
            switch (unaryCond.getOperator()) {
                case UnaryCond.NOT:
                    return !evaluateCond(unaryCond.getCond(), func);
                default:
                    throw new RuntimeException("Unhandled operator");
            }
        } else {
